...
class LotteryGame:
    def __init__(self, master):
        ...
        self.header_frame.grid(
            row=0, column=0, columnspan=10, pady=(20, 30), sticky="ew"
        )
        ...
        for i in range(1, 100):
            ...
            button.grid(
                row=(i) // 10 + 1, column=(i) % 10, padx=5, pady=5, sticky="nsew"
            )
            self.buttons.append(button)

        # Button to draw winning numbers
        draw_button = ttk.Button(
            self.master, text="Tirage", command=self.draw_numbers, style="Draw.TButton"
        )
        draw_button.grid(row=12, column=5, pady=(10, 20), sticky="nsew")

        # Score
        self.score_label = ttk.Label(
            self.master, text="Score: 0", font=("Helvetica", 12), style="Score.TLabel"
        )
        self.score_label.grid(row=13, column=5, sticky="nsew")

        # Button to switch light/dark mode
        mode_button = ttk.Checkbutton(
            self.master,
            text="Dark Mode",
    ...
    def draw_numbers(self):
        if len(self.selected_buttons) == 5:
            ...
            # Displaying results
            result_message = (
                f"Numéros gagnants : {winning_numbers}\nGains : {winnings} points"
            )
            ...
            if len(self.selected_buttons) == 5 and winnings == 50000:
                messagebox.showinfo(
                    "Félicitations !", "Vous avez remporté le jackpot !"
                )
    ...
    def toggle_mode(self):
        ...
        self.header_frame.configure(
            style="HeaderDark.TFrame" if self.dark_mode.get() else "Header.TFrame"
        )

        # Update score label
        self.score_label.configure(
            style="ScoreDark.TLabel" if self.dark_mode.get() else "Score.TLabel"
        )
